...
class ByteSwappingBitField<N> private constructor(private val bitField: IBitField<N, UShort>) :
  ...
  companion object {
    /** Swap the byte order within a [UShort] 1111111100000000 will become 0000000011111111 */
...
class LookupBitField<N, O, T : Any?>
private constructor(
    private val bitField: IBitField<N, O>,
    pairs: Collection<Pair<UInt, T>>,
) : IBitField<N, T> {
  companion object {
    inline fun <N, O, reified T> IBitField<N, O>.lookup(): LookupBitField<N, O, T> where
    T : FieldMapping,
    T : Enum<T> = lookup(enumValues<T>().toList())

    fun <N, O, T : FieldMapping> IBitField<N, O>.lookup(
        mappings: Collection<T>
    ): LookupBitField<N, O, T> = LookupBitField(this, mappings.map { it.bitValue to it })

    fun <N, O, T : Any?> IBitField<N, O>.lookup(
        vararg pairs: Pair<UInt, T>
    ): LookupBitField<N, O, T> = LookupBitField(this, pairs.toList())

    fun <N, O> IBitField<N, O>.asBoolean(): LookupBitField<N, O, Boolean> {
      check(mask.countOneBits() == 1)
      return lookup(1u to true, 0u to false)
    }
  }
